...
polynomial_order = bool(sys.argv[3])
print(f"[{ts_idx}] >>> Polynomial order: {polynomial_order}")
...
elementIDs = [
    -1 * polynomial_order**3,
    38733372 * polynomial_order**3,
    140869280 * polynomial_order**3,
    151935592 * polynomial_order**3,
    245637464 * polynomial_order**3,
    257676220 * polynomial_order**3,
]
...
pointAndCellIds = PointAndCellIds(Input=nek5000_data)
pointAndCellIds.GeneratePointIds = 0
pointAndCellIds.GenerateCellIds = 1
pointAndCellIds.UpdatePipeline()
...
for domain in range(len(domainNames)):
    if filter_domains:
        threshold = Threshold(
            registrationName=f"Threshold_{ts_idx}_{domain}", Input=nek5000_data
        )
        print(
            f"[{ts_idx}] >>> Domain {domain}: {domainNames[domain]}, time={time.time()-t_init:.2f} s"
        )
        print(
            f"[{ts_idx}] >>> Filtering by Element ID, time={time.time()-t_init:.2f} s"
        )
        threshold.Set(
            Scalars=["CELLS", "CellIds"],
            LowerThreshold=elementIDs[domain] + polynomial_order**3,
            UpperThreshold=elementIDs[domain + 1],
        )
        ...
        print(
            f"[{ts_idx}] >>> Checkpoint: 4 ({domain}), time={time.time()-t_init:.2f} s"
        )
        print(f"[{ts_idx}] >>> Resample to Image")

        # Apply Resample To Image filter
        resample = ResampleToImage(
            registrationName=f"Resample_{ts_idx}_{domain}", Input=threshold
        )
        resample.Set(
            UseInputBounds=0,
